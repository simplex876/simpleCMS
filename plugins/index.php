...
/*
=========================================================
BUGFIX for CMSimple 5.18 by ge-webdesign.de 2025-03
Array Plugins with admin.php: is_dir() added (Pluginloader index.php)

MODIFIED FOR CMSimple 5.16 by ge-webdesign.de 2024-04
Ready for php 8.3.4
...
{
	$pm_datafile_path = $pth['folder']['base'] . 'content/_disabled_plugins.txt';
}
else
{
	$pm_datafile_path = './content/_disabled_plugins.txt';
}
...
if ($adm) 
{
	...
	closedir($handle);
    if(is_array($admin_plugins) && count($admin_plugins) > 0) {
		natcasesort($admin_plugins); 
	}
	else $o = '<br><b>Pluginloader: </b>No plugin istalled';
}
...
if(preCallPlugins() != null) str_ireplace('#CMSimple','<span>#</span>CMSimple',preCallPlugins());
...
function PluginSaveForm($form=ARRAY(), $style=ARRAY(), $data=ARRAY(), $hint=ARRAY()) 
{
	...
	if($adm)
	{
		...
		else 
		{
			...
			if($cf['use']['csrf_protection'] == 'true') $saveform .= '<input type="hidden" name="csrf_token" value="' . $_SESSION[$csrfSession] . '">' . "\n";
